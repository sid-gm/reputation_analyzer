...
export async function GET() {
  ...
  const [todayRow] = await db
    .select({ count: count() })
    .from(ingestedItems)
    .where(and(eq(ingestedItems.platform, "google_alerts"), gte(ingestedItems.publishedAt, startOfToday)));

  const [sevenDaysRow] = await db
    .select({ count: count() })
    .from(ingestedItems)
    .where(and(eq(ingestedItems.platform, "google_alerts"), gte(ingestedItems.publishedAt, sevenDaysAgo)));
