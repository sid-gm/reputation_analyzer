...
export async function GET() {
  ...
  const [todayRow] = await db
    .select({ count: count() })
    .from(ingestedItems)
    .where(and(eq(ingestedItems.platform, "twitter"), gte(ingestedItems.publishedAt, startOfToday)));

  const [sevenDaysRow] = await db
    .select({ count: count() })
    .from(ingestedItems)
    .where(and(eq(ingestedItems.platform, "twitter"), gte(ingestedItems.publishedAt, sevenDaysAgo)));

  const [lastPollRow] = await db
    .select({ lastPoll: max(ingestedItems.createdAt) })
    .from(ingestedItems)
    .where(eq(ingestedItems.platform, "twitter"));
